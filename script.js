...
let	trueIndicator = document.createElement("div");



// let colorchanger = document.getElementById("colorvalue");

let output = document.querySelector(".allsvgcontainer");

let outputdragzone = document.querySelector("#dragtarget");

let outputContainer = document.querySelector(".result");


let insertbeforeit = document.getElementById("insertbeforeit")

let dragIndicator = document.getElementById("indicator");
...
let firstZone = document.querySelector(".emptyfordraggingstart");

let lastZone = document.querySelector(".emptyfordraggingend");
...
let exportButton = document.getElementById("bExport");
let printButton = document.getElementById("bPrint");

let infoButton = document.getElementById("bInfo");
...
let hiddenPrintZone = document.getElementById("printmodeonlyImg");
...
function ChangeSVGColors(value = null){
...
function addDraggingEvent(event){



	outputdragzone.ondragend = whenDeDragging;
	// outputdragzone.ondragleave = whenDeDragging;
	// outputdragzone.onmouseup = whenDeDragging;
	// return;			
	
	let elements = outputdragzone.querySelectorAll(".block img");
...
function whenDeDragging(event){
	// console.log(event.srcElement,event.target)
	dragPathBool = false;
	dragPointBool = false;
}

function whenDragging(event){
	...
	if(dragPathBool || dragPointBool){
		return
		if(typeToInsert == 0){
			trueIndicator.style.backgroundImage="url(blocks/pathline.svg)";
		}
		else{
			trueIndicator.style.backgroundImage="url(blocks/pointempty.svg)";

		}

		trueIndicator.className = "indicator";



		

		// console.warn(output.children,insertAfter);

		let isout = true;

		if(!isemptyzone){
				if(insertingBefore){
					output.insertBefore(trueIndicator,target);
				}
				else{
					output.insertBefore(trueIndicator,target.nextElementSibling);			
				}
		}
		else{
			// Last element
			if(insertingBefore){
					output.insertBefore(trueIndicator,lastZone);			
			}
			else{
					output.insertBefore(trueIndicator,firstZone.nextElementSibling);
			}
		}
...
function manageClick(event){
	...
	if(CURRENTLY_DOING == deleting){
		...
		if(trueCurrentElement.className != "blockcontainer line InsertBeforeit"){output.removeChild(trueCurrentElement);}
		if(output.childElementCount == 1){
			insertbeforeit.style.display = "block";
		}
		return;
...
outputOndragstart = function(event){
	LAST_X = event.pageX;
	whenDragging(event);
}
...
outputOndragend = function(event){
	...
	// element.offsetLeft - output.offsetLeft
	...
	if( afterElement && afterElement.className == "emptyfordraggingend"){
		output.insertBefore(element,afterElement.previousElementSibling);
		return;
	}
	else if( beforeElement && beforeElement.className == "emptyfordraggingstart"){
		output.insertBefore(element,beforeElement.nextElementSibling);
		return;
	}

	// if( beforeElement == lastZone || afterElement == lastZone || event.target == lastZone){
	// 		output.insertBefore(element,lastZone);
	// 		return
	// }
	// if( beforeElement == firstZone || afterElement == firstZone || event.target == firstZone){
	// 		output.insertBefore(element,firstZone.nextElementSibling);
	// 		return
	// }
	...
	// if(afterElement == firstZone){
	// 	afterElement = firstZone;
	// }
	...
	output.insertBefore(element,beforeElement);
			...
			// output.replaceChild(element,element.nextElementSibling);
	...
	// 	output.replaceChild(element,element.nextElementSibling);
	// 	output.insertBefore(nextElementToSwap,element);

	// 	// let nextElementToSwap = elementToSWAP.cloneNode(true);
	// 	// output.replaceChild(element,elementToSWAP);
	// 	// output.insertBefore(nextElementToSwap,element);
	// }
	// // Swapping with previous element
	// else if(deltaX < -3){
	// 	output.insertBefore(element,element.previousElementSibling);
	// 	// output.insertBefore(element,elementToSWAP);
	// }
...
asideContainer.onclick = function(event){
...
// manageConnection.onclick = function(event){
// 	if(manageConnection.className == "squared modernbutton dragger"){
// 		manageConnection.className = "squared modernbutton dragger checked";
// 		SWITCHING_CONNECTION = true;
// 		// document.body.className="deleting";
// 	}
// 	else{
// 		manageConnection.className="squared modernbutton dragger";
...
function removeIndicator(event = null){
	// return
	if(trueIndicator.parentElement){
			output.removeChild(trueIndicator);
	}
}
...
function outputOnmousemove(event){
	// Not in drawing mode
	...
	if( CURRENTLY_DOING == drawingpath && trueIndicator.className != "indicator line" ){
		trueIndicator.innerHTML = pathHTML.replace("#fcc907",GLOBALColor);
		trueIndicator.className = "indicator line";
	}

	else if( CURRENTLY_DOING == drawingpoint && trueIndicator.className != "indicator point" ){
		trueIndicator.innerHTML = pointHTML.replace("#fcc907",GLOBALColor);
		trueIndicator.className = "indicator point";
	}



	if(target == trueIndicator){return}
	// console.log(target);


	if( mouseX > targetMid ){
		if( target.nextElementSibling && target.nextElementSibling != trueIndicator ){
			output.insertBefore(trueIndicator,target.nextElementSibling);
		}
	}
	else{
		if( target.previousElementSibling && target.previousElementSibling != trueIndicator ){
			output.insertBefore(trueIndicator,target);
		}
	}
...
function outputOnclick(event){
	...
	if(CURRENTLY_DOING == drawingpath){
		...
		if(toAdd){output.insertBefore(toAdd,trueIndicator);}
...
function LoadEventsManager(){
	// outputdragzone.ondragenter = whenDragging;
	outputdragzone.onclick = manageClick;
	// trueIndicator.onclick = outputOnclick;
	// output.ondragstart = outputOndragstart;
	output.ondragend = outputOndragend;
	output.onmouseleave = removeIndicator;
	output.onmousemove = outputOnmousemove;
}
...
setInterval( function(){ saveLocalStorage(); } , 1000 * SAVE_INTERVAL );
// Save before closing
...
function exportPicture(event = null,format = 0){
	document.body.style.overflow = "visible";
	output.style.overflowX = "visible";
	outputContainer.style.overflowX = "visible";
	outputContainer.style.maxWidth = "unset";
	let promised;
	let filename;
	...
	promised.finally(function(){
		...
		output.style.overflowX = "";
		outputContainer.style.overflowX = "";
		outputContainer.style.maxWidth = "";
...
function preparePrint(event){
	...
	promised.then(function (dataUrl) { 
		...
		hiddenPrintZone.src = dataUrl;
		hiddenPrintZone.onload = function(){print()};
...
exportButton.onclick = exportPicture;
infoButton.onclick = function(event){aboutWindow.style.display = "flex";}

printButton.onclick = preparePrint;




let lineNameContainer = outputdragzone.querySelector(".linename .connectionline");
function tryImport(){
